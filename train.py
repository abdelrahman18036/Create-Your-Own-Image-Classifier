...
# Define the arguments of the command line
...
def train_model(
    ...
    # Criterion description, optimization and  scheduler.
    # Note that only gradient parameters are optimised.
    ...
    for epoch in range(epochs):
        ...
        # Every Eroch has a stage of training and validation
    ...
    print('Completed training in {:.0f}m {:.0f}s'.format(
        time_elapsed // 60, time_elapsed % 60))
    print('Best val Acc: {:4f}'.format(best_acc))
    # Load best model weights
    model.load_state_dict(best_model_wts)
    # Store class to idx in a pattern
    model.class_to_idx = image_datasets['train'].class_to_idx    
    if checkpoint:
        print ('Checkpoint saving to:', checkpoint) 
        checkpoint_dict = {
            'arch': arch,
...
if args.data_dir:    
    ...
    # Default for the preparation, validation and test sets transforms
